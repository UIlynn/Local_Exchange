         ...
         $.getJSON(`./data/${cityName}/${townName}/store.json`, function(data){
             ...
             if (currentCategoryName){
                 data.forEach(element=>{
                     if (element.INDUTYPE_NM.indexOf(currentCategoryName) > -1 || element.CMPNM_NM.indexOf(currentCategoryName) > -1){
                         // console.dir(element);
                         drawMarker(element,cityName,townName);
                     }
                 })
             } else{
                 // 마커 표시
                 data.forEach(element=>{
                     // console.log(`drawMarker(${element},${cityName},${townName})`)
                     drawMarker(element,cityName,townName);
                 })    
             }
         })
     ...
     function getStoreJson(cityName,townName){
         ...
         fetch(`./data/${cityName}/${townName}/store.json`).then(
             response => {
                 // console.log(response)
                 if (response.ok){ // 파일이 존재하지 않으면 skip
                     // console.log(`${cityName}/${townName}.json 가져와서 찍기 실행`)
                     jsonToMarker(cityName, townName)
                 }
             }
         ) 
     }
